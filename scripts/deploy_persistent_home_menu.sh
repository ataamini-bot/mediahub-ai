...
# Bot-only release for:
# - the persistent, scrollable bottom main menu;
# - the Telegram command-menu /menu recovery action;
# - removing duplicate inline home menus from payments and admin flows;
# - reading uploaded audio/video files through the local Telegram Bot API.
# No database migration, backend, worker, monitor, or Telegram API restart is
# required.
...
if ! [[ "$target_commit" =~ ^[0-9a-f]{40}$ ]]; then
  printf 'USAGE: bash scripts/deploy_persistent_home_menu.sh <full-target-commit>\n'
  exit 1
fi
...
rollback_tag="mediahub-ai-bot:rollback-persistent-home-menu-$(date -u +%Y%m%dT%H%M%SZ)"
...
configuration = {
    ...
    "buttons": {},
    "custom_buttons": [
        {"id": item, "label_en": f"Custom {item}", "is_active": True}
        for item in range(1, 10)
    ],
}
keyboard = build_home_reply_keyboard("en", configuration=configuration)
labels = [button.text for row in keyboard.keyboard for button in row]
assert keyboard.is_persistent is True
assert all(f"Custom {item}" in labels for item in range(1, 10))
...
async def verify() -> None:
    with TemporaryDirectory() as directory:
        ...
        assert await _download_telegram_file(local_bot, "file-id", destination) == "local-path"
        assert destination.read_bytes() == b"audio-smoke"

    command_bot = SimpleNamespace(
        get_my_commands=AsyncMock(return_value=[SimpleNamespace(command="start")]),
        set_my_commands=AsyncMock(),
        set_chat_menu_button=AsyncMock(),
    )
    await configure_telegram_menu_button(command_bot)
    commands = command_bot.set_my_commands.await_args.args[0]
    assert [command.command for command in commands] == ["start", "menu"]
    assert isinstance(
        command_bot.set_chat_menu_button.await_args.kwargs["menu_button"],
        MenuButtonCommands,
    )


asyncio.run(verify())
print("PERSISTENT_HOME_MENU_CODE=OK")
