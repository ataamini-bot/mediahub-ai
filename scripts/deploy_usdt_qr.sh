...
expected_base="8e7052d6775483bfd92a94b6d90d72db4ac304c9"
expected_branch="feature/admin-foundation"
target_commit="${1:-}"

if ! [[ "$target_commit" =~ ^[0-9a-f]{40}$ ]]; then
  printf 'USAGE: bash scripts/deploy_usdt_qr.sh <full-target-commit>\n'
  exit 1
fi
...
rollback_tag="mediahub-ai-bot:rollback-usdt-qr-$(date -u +%Y%m%dT%H%M%SZ)"
...
from app.handlers.payments import _payment_destination_text
from app.utils.payment_qr import build_usdt_address_qr

address = "TVjsPublicLBankDepositAddress123"
qr_png = build_usdt_address_qr(address)
assert qr_png.startswith(b"\x89PNG\r\n\x1a\n")
caption = _payment_destination_text(
    {
        "label": "Global",
        "duration_days": 30,
        "price": "3",
...
assert "Scan the QR code" in caption
print("USDT_QR=OK")
PY
